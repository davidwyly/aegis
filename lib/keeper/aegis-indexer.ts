  ...
  for (const rawLog of logs) {
    try {
      ...
      switch (decoded.eventName) {
        ...
        case "CaseAppealable": {
          await applyStatusUpdate(cfg, decoded.args.caseId, "appealable_resolved")
          break
        }
        ...
        case "AppealPanelSeated": {
          await applyAppealPanelSeated(cfg, decoded.args)
          break
        }
        case "AppealCommitted": {
          await applyAppealCommitted(cfg, decoded.args)
          break
        }
        case "AppealRevealed": {
          await applyAppealRevealed(cfg, decoded.args)
          break
        }
        case "AppealUpheld":
        case "AppealOverturned":
        case "AppealStalled": {
          // Each of these is followed by either CaseResolved or
          // CaseDefaultResolved emitted in the same tx — those handlers
          // sweep up the median + resolutionTxHash. Nothing extra here.
          break
        }
...
type DecodedLog =
  ...
  | { eventName: "CaseAppealable"; args: any }
  | { eventName: "AppealRequested"; args: any }
  | { eventName: "AppealPanelSeated"; args: any }
  | { eventName: "AppealCommitted"; args: any }
  | { eventName: "AppealRevealed"; args: any }
  | { eventName: "AppealUpheld"; args: any }
  | { eventName: "AppealOverturned"; args: any }
  | { eventName: "AppealStalled"; args: any }
